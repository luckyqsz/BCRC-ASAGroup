...
import os
import pickle
import scipy.io  # to save visualized samples to disk
import numpy as np

from torch.autograd import Variable
from torch import optim
from model import G12, G21  # 1 for MNIST, 2 for SVHN
...
class Solver(object):
    def __init__(self, config, svhn_loader, mnist_loader):
        ...
        self.train_iters = config.train_iters  # 40000
    ...
    def build_model(self):  # instantiate model
    ...
    def merge_images(self, sources, targets, k=10):
        ...
        row = int(np.sqrt(self.batch_size))  # 8 = sqrt(64)
        merged = np.zeros([3, row*h, row*w*2])  # 全图大小, 两个域挨着显示
        for idx, (s, t) in enumerate(zip(sources, targets)):  # side-by-side display, 64
            i = idx // row  # 整除
            j = idx % row  # 取余
            merged[:, i*h:(i+1)*h, (j*2)*h:(j*2+1)*h] = s
            merged[:, i*h:(i+1)*h, (j*2+1)*h:(j*2+2)*h] = t
        return merged.transpose(1, 2, 0)  # change the location of channel
    ...
    def to_data(self, x):
        """Converts variable to numpy."""  # numpy does not work on GPU
        if torch.cuda.is_available():
            x = x.cpu()
        return x.data.numpy()
    ...
    def train(self):
        svhn_iter = iter(self.svhn_loader)  # 返回迭代器对象，这样可以使用 .next()方法
        mnist_iter = iter(self.mnist_loader)
        iter_per_epoch = min(len(svhn_iter), len(mnist_iter))
        # the length of this two datasets mismatch.
        # 可以把这个数据流看做是一个有序序列，但我们却不能提前知道序列的长度，
        # 只能不断通过next()函数实现按需计算下一个数据，
        # 所以Iterator的计算是惰性的，只有在需要返回下一个数据时它才会计算。
        
        # fixed mnist and svhn for sampling, visualize the same sample to observe the performance.
        fixed_svhn = self.to_var(svhn_iter.next()[0])  # 将数据搬往GPU
        fixed_mnist = self.to_var(mnist_iter.next()[0])  # batchsize is 64
        
        # loss if use_labels = True
        criterion = nn.CrossEntropyLoss()
        
        for step in range(self.train_iters+1):  # 40000 step
            # reset data_iter for each epoch
            if (step+1) % iter_per_epoch == 0:  # due to this two dataset length mismatch
                mnist_iter = iter(self.mnist_loader)  # start from index 0
            ...
            if self.use_labels:
                ...
                d1_loss = torch.mean((out-1)**2)  # dimension broadcasting, 1 is groundtruth 
            ...
            if self.use_labels:
                ...
                d1_loss = torch.mean(out**2)  # acturally (out-0)**2, 0 is groundtruth 
            ...
            if self.use_labels:
                ...
                g_loss = torch.mean((out-1)**2)   # GAN loss

            if self.use_reconst_loss:
                g_loss += torch.mean((mnist - reconst_mnist)**2)  # reconstruction loss
            ...
            # print the log info
            if (step+1) % self.log_step == 0:  # every 10 step print log info
            ...
            if (step+1) % self.sample_step == 0:
                ...
                mnist, fake_mnist = self.to_data(fixed_mnist), self.to_data(fake_mnist)  # to cpy numpy
