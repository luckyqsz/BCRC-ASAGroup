""" CycleGAN application: MNIST & SVHN neural style transfer.
"""
...
def str2bool(v):
    return v.lower() in ('true')  # return bool type
    # 要判断特定的值是否存在列表中，可使用关键字in,
    # 判断特定的值不存在列表中，可使用关键字not in

def main(config):  # config is a dict
    ...
    if config.mode == 'train':
        ...
        solver.sample()
        # AttributeError: 'Solver' object has no attribute ‘sample’
