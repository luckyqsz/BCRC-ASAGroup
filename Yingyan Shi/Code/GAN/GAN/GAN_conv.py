""" Implement original GAN using conv in MNIST dataset with PyTorch.
"""
import torch
...
class generator(nn.Module):
    def __init__(self,input_size,num_feature):
        ...
        # using fc as the first layer in order to enlarge the feature map, so utilize conv layers later.
    ...
    def forward(self, x):
        x=self.fc(x)
        x=x.view(x.size(0),1,56,56)  # shape transformation in order to perform convolution operation.
        x=self.br(x)
        x=self.gen(x)
        return x
...
if __name__=="__main__":
    ...
    z_dimension=100
    D=discriminator()
    G=generator(z_dimension,3136) #1*56*56 = fc dimensions
