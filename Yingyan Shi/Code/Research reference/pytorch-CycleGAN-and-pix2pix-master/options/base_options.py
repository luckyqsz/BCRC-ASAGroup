""" Training and testing mode shared options and arguments.
"""
import argparse
import os
...
class BaseOptions():
    ...
    def gather_options(self):
        ...
        if not self.initialized:
            parser = argparse.ArgumentParser(
                formatter_class=argparse.ArgumentDefaultsHelpFormatter)  # create a parse instance
            parser = self.initialize(parser)  # formatter_class is the format of printed help infomation

        # get the basic options
        opt, _ = parser.parse_known_args()  # assign the parsed arguments to an object, "opt"
        # parser.parse_known_args() will accept all arguments without raising error even if there are more arguments than it defines.
        # And then it will put the undefined arguments into a dict as well.
    ...
    def print_options(self, opt):
        message = ''
        message += '----------------- Options ---------------\n'
        for k, v in sorted(vars(opt).items()):  # built-in function returns a dict of object's attributes and their values.
            comment = ''
            default = self.parser.get_default(k)
            if v != default:
                comment = '\t[default: %s]' % str(default)
                    message += '{:>25}: {:<30}{}\n'.format(str(k), str(v), comment)  # {:>25}取25位右对齐，{:<30}取30位左对齐，因为各参数名长度不一样，需要这样对齐才美观。
        message += '----------------- End -------------------'
        print(message)
